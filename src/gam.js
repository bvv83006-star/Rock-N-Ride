...
startBtn.addEventListener('click', async () => {
  ...
  // TIMEOUT: if loading takes over 8 seconds, force-start with fallback boxes
  let forceStarted = false;
  const timeoutId = setTimeout(() => {
    if(forceStarted) return;
    forceStarted = true;
    console.warn('⚠️ Model load timeout — starting with fallback boxes');
    loadingEl.textContent = 'TIMEOUT — STARTING…';
