...
export function preloadAllCars(onProgress){
  return Promise.all(CAR_CLASSES.map(c => new Promise(resolve => {
    try {
      loader.load(c.model,
        (gltf) => {
          try {
            ...
            console.warn('⚠️ Post-process failed for', c.id, e);
          }
          loadedCount++;
        ...
        undefined,
        (err) => {
          console.warn('❌ Failed to load', c.model, err);
          loadedCount++;
          if(onProgress) onProgress(loadedCount, CAR_CLASSES.length);
          resolve();
        }
      ...
      console.warn('❌ Exception loading', c.model, e);
      loadedCount++;
      if(onProgress) onProgress(loadedCount, CAR_CLASSES.length);
      resolve();
    }
...
export function makeCar(classId, colorHex, opts = {}){
  ...
  let car;
  if(!template){
    car = makeFallbackBox(colorHex);
    car.userData.classId = cls.id;
    return car;
  }

  try {
    car = template.clone(true);
  } catch(e) {
    console.warn('Clone failed for', cls.id, '— using fallback box');
    car = makeFallbackBox(colorHex);
    car.userData.classId = cls.id;
    return car;
  }
  ...
  try {
    ...
    if(biggestMesh && biggestMesh.material){
      ...
      car.userData.bodyMaterial = biggestMesh.material;
    }
  } catch(e) {
    console.warn('Recolor failed for', cls.id);
  }
