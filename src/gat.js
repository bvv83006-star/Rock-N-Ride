...
export function buildWorld(scene){
  ...
  const runwayMat = new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 0.92 });
  const runway = new THREE.Mesh(new THREE.PlaneGeometry(320, 44), runwayMat);
  runway.rotation.x = -Math.PI/2;
  runway.position.set(AIRPORT_X, 0.02, AIRPORT_Z);
  runway.receiveShadow = true;
  scene.add(runway);

  const dashMat = new THREE.MeshBasicMaterial({ color: 0xffffff });
  for(let i = -18; i <= 18; i++){
    const dash = new THREE.Mesh(new THREE.PlaneGeometry(6, 0.7), dashMat);
    dash.rotation.x = -Math.PI/2;
    dash.position.set(AIRPORT_X + i * 8, 0.03, AIRPORT_Z);
    scene.add(dash);
  }
  for(const off of [-20, 20]){
    const edge = new THREE.Mesh(new THREE.PlaneGeometry(320, 0.5), dashMat);
    edge.rotation.x = -Math.PI/2;
    edge.position.set(AIRPORT_X, 0.03, AIRPORT_Z + off);
    scene.add(edge);
  }

  const apron = new THREE.Mesh(new THREE.PlaneGeometry(140, 100), runwayMat);
  apron.rotation.x = -Math.PI/2;
  apron.position.set(AIRPORT_X - 20, 0.02, AIRPORT_Z + 80);
  scene.add(apron);

  const terminalMat = new THREE.MeshStandardMaterial({ color: 0x8899aa, roughness: 0.6, metalness: 0.3 });
  const glassTowerMat = new THREE.MeshStandardMaterial({ color: 0x5a90c8, roughness: 0.1, metalness: 0.7, transparent: true, opacity: 0.75 });

  const terminal = new THREE.Mesh(new THREE.BoxGeometry(120, 12, 30), terminalMat);
  terminal.position.set(AIRPORT_X - 20, 6, AIRPORT_Z + 110);
  terminal.castShadow = true; terminal.receiveShadow = true;
  scene.add(terminal);

  const glassStrip = new THREE.Mesh(new THREE.BoxGeometry(118, 6, 1), glassTowerMat);
  glassStrip.position.set(AIRPORT_X - 20, 6, AIRPORT_Z + 95);
  scene.add(glassStrip);

  const towerBase = new THREE.Mesh(new THREE.BoxGeometry(10, 40, 10), terminalMat);
  towerBase.position.set(AIRPORT_X + 20, 20, AIRPORT_Z + 110);
  towerBase.castShadow = true;
  scene.add(towerBase);

  const towerTop = new THREE.Mesh(new THREE.BoxGeometry(16, 8, 16), glassTowerMat);
  towerTop.position.set(AIRPORT_X + 20, 44, AIRPORT_Z + 110);
  scene.add(towerTop);

  const towerRoof = new THREE.Mesh(new THREE.BoxGeometry(17, 1, 17), terminalMat);
  towerRoof.position.set(AIRPORT_X + 20, 48.5, AIRPORT_Z + 110);
  scene.add(towerRoof);

  const beacon = new THREE.Mesh(new THREE.SphereGeometry(0.6, 8, 6), new THREE.MeshBasicMaterial({ color: 0xff2222 }));
  beacon.position.set(AIRPORT_X + 20, 51, AIRPORT_Z + 110);
  scene.add(beacon);

  const hangarMat = new THREE.MeshStandardMaterial({ color: 0x445566, roughness: 0.7, metalness: 0.4 });
  for(let i = 0; i < 3; i++){
    const hx = AIRPORT_X - 70 + i * 34;
    const hangar = new THREE.Mesh(new THREE.BoxGeometry(30, 12, 30), hangarMat);
    hangar.position.set(hx, 6, AIRPORT_Z + 130);
    hangar.castShadow = true; hangar.receiveShadow = true;
    scene.add(hangar);
    const roof = new THREE.Mesh(new THREE.CylinderGeometry(15, 15, 30, 12, 1, false, 0, Math.PI), hangarMat);
    roof.rotation.z = Math.PI/2;
    roof.rotation.y = Math.PI/2;
    roof.position.set(hx, 12, AIRPORT_Z + 130);
    scene.add(roof);
  }

  // Parked plane
  const planeBodyMat = new THREE.MeshStandardMaterial({ color: 0xdddddd, metalness: 0.5, roughness: 0.3 });
  const planeAccentMat = new THREE.MeshStandardMaterial({ color: 0x2c7cff, metalness: 0.5, roughness: 0.3 });
  const plane = new THREE.Group();
  plane.position.set(AIRPORT_X - 20, 0, AIRPORT_Z + 80);

  const fuselage = new THREE.Mesh(new THREE.CylinderGeometry(2.2, 2.2, 24, 12), planeBodyMat);
  fuselage.rotation.z = Math.PI/2;
  fuselage.position.y = 4;
  fuselage.castShadow = true;
  plane.add(fuselage);

  const nose = new THREE.Mesh(new THREE.SphereGeometry(2.2, 12, 8), planeBodyMat);
  nose.position.set(-12, 4, 0);
  plane.add(nose);

  const wing = new THREE.Mesh(new THREE.BoxGeometry(4, 0.5, 22), planeBodyMat);
  wing.position.set(-2, 4, 0);
  plane.add(wing);

  const tail = new THREE.Mesh(new THREE.BoxGeometry(3, 5, 0.5), planeAccentMat);
  tail.position.set(11, 7, 0);
  plane.add(tail);

  const tailH = new THREE.Mesh(new THREE.BoxGeometry(2, 0.4, 8), planeBodyMat);
  tailH.position.set(11, 5, 0);
  plane.add(tailH);

  for(const z of [-5, 5]){
    const eng = new THREE.Mesh(new THREE.CylinderGeometry(1, 1, 3, 8), planeAccentMat);
    eng.rotation.z = Math.PI/2;
    eng.position.set(-3, 3.2, z);
    plane.add(eng);
  }
  scene.add(plane);

  // ============================================================
  // BIG BOWL — 700 GLITCH PAD
  // ============================================================
  const BOWL_X = 520;
  const BOWL_Z = -300;
  const BOWL_RADIUS = 60;
  ...
  for(let r = 0; r < rings; r++){
    const t0 = r / rings;
    const t1 = (r + 1) / rings;
    const radius0 = BOWL_RADIUS * (1 - t0 * 0.15);
    const radius1 = BOWL_RADIUS * (1 - t1 * 0.15);
    const y0 = t0 * BOWL_HEIGHT;
    const y1 = t1 * BOWL_HEIGHT;
    for(let s = 0; s < segments; s++){
      const a0 = (s / segments) * Math.PI * 2;
      const a1 = ((s + 1) / segments) * Math.PI * 2;
      const x0 = Math.cos(a0) * radius0;
      const z0 = Math.sin(a0) * radius0;
      const x1 = Math.cos(a1) * radius0;
      const z1 = Math.sin(a1) * radius0;
      const x2 = Math.cos(a1) * radius1;
      const z2 = Math.sin(a1) * radius1;
      const x3 = Math.cos(a0) * radius1;
      const z3 = Math.sin(a0) * radius1;
      ...
      const panel = new THREE.Mesh(geo, bowlMat);
      panel.position.set(BOWL_X, 0, BOWL_Z);
  ...
  // Bowl top rim glow
  const rimGeo = new THREE.TorusGeometry(BOWL_RADIUS * 0.85, 0.4, 8, 64);
  const rimMat = new THREE.MeshStandardMaterial({ color: 0xffd400, emissive: 0xffd400, emissiveIntensity: 1.6 });
  const rim = new THREE.Mesh(rimGeo, rimMat);
  rim.rotation.x = Math.PI/2;
  rim.position.set(BOWL_X, BOWL_HEIGHT, BOWL_Z);
  scene.add(rim);

  // Access ramp
  const accessRamp = new THREE.Mesh(
    new THREE.BoxGeometry(20, 0.6, 40),
    new THREE.MeshStandardMaterial({ color: 0x3a3a42, roughness: 0.8 })
  );
  accessRamp.position.set(BOWL_X, 8, BOWL_Z + BOWL_RADIUS * 0.7);
  accessRamp.rotation.x = -Math.atan2(16, 40);
  accessRamp.receiveShadow = true;
  scene.add(accessRamp);

  // Yellow glow pad (glitch trigger zone)
  const glitchPad = new THREE.Mesh(
    new THREE.CircleGeometry(6, 32),
    new THREE.MeshStandardMaterial({ color: 0xffd400, emissive: 0xffd400, emissiveIntensity: 1.2, transparent: true, opacity: 0.7 })
  );
  glitchPad.rotation.x = -Math.PI/2;
  glitchPad.position.set(BOWL_X, 0.05, BOWL_Z);
  scene.add(glitchPad);

  // Glowing pillars around the pad
  for(let i = 0; i < 6; i++){
    const a = (i / 6) * Math.PI * 2;
    const p = new THREE.Mesh(
      new THREE.CylinderGeometry(0.4, 0.4, 8, 8),
      new THREE.MeshStandardMaterial({ color: 0xffd400, emissive: 0xffd400, emissiveIntensity: 1.8 })
    );
    p.position.set(BOWL_X + Math.cos(a) * 10, 4, BOWL_Z + Math.sin(a) * 10);
    scene.add(p);
  }

  // Expose bowl data to game.js
  window.__BOWL__ = { x: BOWL_X, z: BOWL_Z, radius: BOWL_RADIUS, height: BOWL_HEIGHT };
