...
userAvatarEl.textContent = playerName.charAt(0).toUpperCase();
...
async function tryLandscape(){
  try {
    ...
    console.log('Landscape lock N/A (iOS)');
  }
}
...
function updateAbilityButton(){
  ...
  if(!btn) return;
  const cls = getCarClass(P.carClass);
  if(cls.ability === 'none'){ btn.style.opacity = '0.3'; btn.textContent = '—'; }
...
if(socket){
  socket.on('connect', () => {
    socket.emit('setInfo', { name: playerName, color: playerColor });
  });
...
// Paint swatches — mark saved colour as selected
paintRow.querySelectorAll('button').forEach(b => {
  b.classList.toggle('selected', b.dataset.color === savedColor);
});
...
function addChatMessage(name, text, mine){
  ...
  // Auto-open chat for 5s when a message arrives
  if(!chatBoxEl.classList.contains('open')){
    chatBoxEl.classList.add('open');
    clearTimeout(chatBoxEl._autoClose);
    chatBoxEl._autoClose = setTimeout(() => {
      if(document.activeElement !== chatInputEl) chatBoxEl.classList.remove('open');
    }, 5000);
  }
}
...
function showBubble(playerId, text){
  ...
  if(chatBubbles[playerId]){
    scene.remove(chatBubbles[playerId].sprite);
    clearTimeout(chatBubbles[playerId].timeout);
  }
  ...
  scene.add(sprite);
  chatBubbles[playerId] = {
    sprite,
    timeout: setTimeout(() => { scene.remove(sprite); delete chatBubbles[playerId]; }, 4000)
  };
}
...
function updatePlayer(dt){
  ...
  if(Math.hypot(P.x - cp.x, P.y - cp.y, P.z - cp.z) < 4.4){ P.score += 1500; P.coins += 25; P.cpIndex = (P.cpIndex + 1) % CHECKPOINTS.length; }
...
startBtn.addEventListener('click', async () => {
  ...
  // Start background music (user gesture unlocks it)
  if(musicOn){
    try { startMusic(); } catch(e){ console.warn('Music fail', e); }
  }
  ...
  try {
    ...
    } catch(e) { console.warn('Player car swap failed:', e); }
...
pauseBtn.addEventListener('click', () => {
  if(!started) return;
  if(!paused){ save.coins += P.coins; saveGame(save); coinCountEl.textContent = Math.floor(save.coins).toLocaleString(); P.coins = 0; }
  setPaused(!paused);
});
