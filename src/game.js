...
async function requestTilt(){
  ...
  try {
    ...
    window.addEventListener('deviceorientation', e => {
      tiltGamma = e.gamma || 0;   // left/right tilt
      tiltBeta = e.beta || 0;     // forward/back tilt
    });
    tiltActive = true;
    console.log('✅ Tilt steering enabled');
...
function updatePlayer(dt){
  ...
  // Tilt steering blends in when no button is pressed
  if(tiltActive && isMobile && steer === 0){
    const dead = 3;
    const g = tiltGamma;
    if(Math.abs(g) > dead){
      steer += Math.max(-1, Math.min(1, (g - Math.sign(g)*dead) / 28));
    }
  }
  ...
  if(window.__BOWL__){
    ...
    const dxB = P.x - bowl.x;
    const dzB = P.z - bowl.z;
