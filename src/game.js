...
import {
  STREETS, STREET_WIDTH, WORLD_HALF, HWY_Y, HWY_HALF,
  ...
  for(const s of STREETS){
    for(let p = -360; p <= 360; p += 34){
      ...
      if(Math.random() < 0.3){
        ...
        scene.add(c);
        parkedCars.push({
          type: 'parked', mesh: c, taken: false, respawnAt: 0,
          x: c.position.x, z: c.position.z, rot: c.rotation.y
        });
      }
    }
  }
...
function updatePlayer(dt){
  ...
  // Nitro drain skips during afterburner
...
function updateTraffic(dt){
  ...
  for(const t of trafficCars){
    if(t.taken){
      if(performance.now() > t.respawnAt) respawnVehicle(t);
      continue;
    }
    if(sirenActive){
...
startBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    console.error(err);
    loadingEl.textContent = 'FAILED TO LOAD MODELS';
  }
